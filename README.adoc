...
public class BindingInitializer {
  ...
  public void initialize() {
    try {
      binder.configuration().addHost("somehost.somedomain").setUsername("myuser").setPassword("mypassword");
      binder.initialize();
    } catch (IOException e) {
...
In case you have to support two different servers, create a binder implementation for each host and initialize
them in one single binding initializer:
...
public class BindingInitializer {
  ...
  public void initialize() {
    try {
      binderOne.configuration().addtHost("hostOne.somedomain").setUsername("myuser").setPassword("mypassword");
      binderTwo.configuration().addtHost("hostTwo.somedomain").setUsername("myuser").setPassword("mypassword");
