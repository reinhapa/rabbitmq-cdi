...
/**
 * <p>
 * Binds incoming CDI events to queues and outgoing CDI events to exchanges of a
 * broker.
 * </p>
 *
 * <p>
 * Inherit from this class and override its {@link #bindEvents()} method to
 * create bindings.
 * </p>
 *
 * <p>
 ...
 * 	    .withRoutingKey(&quot;myRoutingKey&quot;).withPublisherTransactions();
 * }
 * </pre>
 *
 * <p>
 * To initialize the event bindings, inject the instance of this class and call
 * {@link #initialize}. In a web application, you would normally do this in a
 * context listener on application startup <b>after</b> your CDI framework was
 * initialized.
 * </p>
 *
...
public abstract class EventBinder {
    ...
    /**
     * <p>
     * Extend {@link EventBinder} and implement this method to create the event
     * bindings for your application.
     * </p>
     *
     * <p>
     * <b>Binder example:</b>
    ...
    /**
     * <p>
     * Initializes the event binder and effectively enables all bindings created
     * in {@link #bindEvents()}.
     * </p>
     *
     * <p>
     * Inject your event binder implementation at the beginning of your
     * application's lifecycle and call this method. In web applications, a good
     * place for this is a ServletContextListener.
     * </p>
     *
     * <p>
     * After this method was successfully called, consumers are registered at
     * the target broker for every queue binding. Also, for every exchange
     * binding messages are going to be published to the target broker.
     * </p>
     *
     * @throws IOException
     *             if the initialization failed due to a broker related issue
     */
	...
	Event<Object> eventControl = (Event<Object>) remoteEventControl
		.select(queueBinding.eventType);
	@SuppressWarnings("unchecked")
	Instance<Object> eventPool = (Instance<Object>) remoteEventPool
		.select(queueBinding.eventType);
	EventConsumer consumer = new EventConsumer(queueBinding.eventType,
		eventControl, eventPool);
	consumerContainer.addConsumer(consumer, queueBinding.queue,
		queueBinding.autoAck);
	...
	/**
	 * Binds an event to the given queue. On initialization, a consumer is
	 * going to be registered at the broker that is going to fire an event
	 * of the bound event type for every consumed message.
	 *
	 * @param queue
	 *            The queue
	...
	/**
	 * Binds an event to the given exchange. After initialization, all fired
	 * events of the bound event type are going to be published to the
	 * exchange.
	 *
	 * @param exchange
	 *            The exchange
	...
	QueueBinding(Class<?> eventType, String queue) {
	    ...
	    LOGGER.info("Binding created between queue {} and event type {}",
		    queue, eventType.getSimpleName());
	}

	/**
	 * <p>
	 * Sets the acknowledgement mode to be used for consuming message to
	 * automatic acknowledges (auto acks).
	 * </p>
	 *
	 * <p>
	 * If auto acks is enabled, messages are delivered by the broker to its
	 * consumers in a fire-and-forget manner. The broker removes a message
	 * from the queue as soon as its is delivered to the consumer and does
	 * not care about whether the consumer successfully processes this
	 * message or not.
	 * </p>
	 * 
	 * @return the queue binding
	...
	public QueueBinding autoAck() {
	    this.autoAck = true;
	    LOGGER.info("Auto acknowledges enabled for event type {}",
	...
	ExchangeBinding(Class<?> eventType, String exchange) {
	    ...
	    LOGGER.info("Binding created between exchange {} and event type {}",
		    exchange, eventType.getSimpleName());
	}
	...
	public ExchangeBinding withRoutingKey(String routingKey) {
	    ...
	    LOGGER.info("Routing key for event type {} set to {}",
		    eventType.getSimpleName(), routingKey);
	    return this;
	}
	 ...
	 * @return the exchange binding
	 */
	public ExchangeBinding withProperties(
		AMQP.BasicProperties basicProperties) {
	    this.basicProperties = basicProperties;
	    LOGGER.info("Publisher properties for event type {} set to {}",
		    eventType.getSimpleName(), basicProperties.toString());
	    return this;
	}
    }
}